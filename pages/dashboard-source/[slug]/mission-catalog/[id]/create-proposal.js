...
const CreateProposal = () => {
	...
	const handleSubmit = async () => {
		...
		if (formValue.amountType === 'fixed-price') {
			...
			if (milestones.length < 1) {
				setFormError({ ...formError, amountType: 'Milestones length must be more 1' });
				return true;
			}
		}
		setFetch(true);
		try {
			...
			const res = await addProposal(objToSubmit);
			router.back();
		} catch (e) {
			CustomNotice({
				content: e.response?.data?.message,
